...
## `chain_lost` is the chain this hit just broke (0 if it broke nothing).
signal hit_scored(peg: Node2D, points: int, mult_gain: int, relationship: int, chain: int, chain_lost: int)
